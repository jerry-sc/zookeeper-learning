...
/**
 * 在服务端，存在两个 watchmanager 分别对应数据变更的Watcher 和 子节点变更的 Watcher
 *
...
class WatchManager {
    ...
    /**
     * WatchManager 在内部管理两个存储结构，从两个不同维度维护Watcher
     *
     * 此结构 是从数据节点路径的粒度来托管Watcher
     */
     ...
     * WatchManager 在内部管理两个存储结构，从两个不同维度维护Watcher
     *
     * 此结构是从Watcher的粒度来控制事件触发需要触发的数据节点
     */
